...
export default function ReportKbmlPage() {
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/report/kbml/register", {
        method: "POST",
  ...
  const handleSendSms = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/report/kbml/send-sms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleVerify = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/report/kbml/verify-sms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
          ...
          <div className="backdrop-blur-sm rounded-[2px] p-4" style={cardStyle}>
            <div className="text-center mb-4">
              ...
              <p className="text-xs mt-3 leading-relaxed text-left" style={{ color: "#eee" }}>
                ...
                {step === "phone" && "本レポートの内容は機密性が高いため、ご本人確認を必須としております。認証コードをSMSでお送りしますので、電話番号を入力して次へ進んでください。"}
