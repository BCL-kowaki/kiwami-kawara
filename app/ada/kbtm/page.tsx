...
export default function AdaKbtmPage() {
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/ada/kbtm/register", {
        method: "POST",
  ...
  const handleSendSms = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/ada/kbtm/send-sms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleVerify = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/ada/kbtm/verify-sms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      try {
        ...
        if (typeof window !== "undefined" && (window as unknown as { gtag?: (...args: unknown[]) => void }).gtag) {
          (window as unknown as { gtag: (...args: unknown[]) => void }).gtag("event", "ada_report_verify_complete", {
            sender,
              ...
              <p className="text-xs mt-3 leading-relaxed text-left text-white">
                {step === "form" && (
                  <>
                    ...
                    過去の栄光やステーキング報酬だけを頼りに、ADAを握り続けていませんか？<br /><br />
                    膨大なマクロ経済情報とADA固有の個別材料を網羅した精密データをもとに、AI分析エンジン&rdquo;極&rdquo;が冷徹に解剖。<br /><br />
                    感情を一切排除し、現在の流動性枯渇やガバナンスの機能不全を突き詰めた結果、二度と浮上できない「衰退へのシナリオ」が見えてきました。<br /><br />
                    時価総額という&ldquo;幻影&rdquo;の裏で、市場はすでに「無関心」という審判を下しています。<br /><br />
                    まだ当レポートの内容を確認していない方は、今すぐお受け取りください。<br /><br />
                    以下をご入力のうえ、免責事項に同意してお申し込みください。
                  </>
                )}
                {step === "phone" &&
