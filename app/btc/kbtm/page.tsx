...
export default function BtcKbtmPage() {
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/btc/kbtm/register", {
        method: "POST",
  ...
  const handleSendSms = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/btc/kbtm/send-sms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleVerify = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/btc/kbtm/verify-sms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
            ...
            <div className="p-[2px] rounded-[2px] shadow-2xl" style={borderStyle}>
              <div className="backdrop-blur-sm rounded-[2px] p-5 text-center" style={cardStyle}>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  投資のKAWARA版BTC特別レポートのお申し込みありがとうございます。
                  </p>
