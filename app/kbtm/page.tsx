...
export default function KbtmPage() {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await fetch("/api/kbtm/submit", {
                method: "POST",
                headers: {
            ...
            if (result.ok) {
                try {
                    const sender = localStorage.getItem("sender_code") || "(none)";
                    if (typeof window !== "undefined" && (window as any).gtag) {
                        (window as any).gtag("event", "kiwami_submit", {
                            sender,
