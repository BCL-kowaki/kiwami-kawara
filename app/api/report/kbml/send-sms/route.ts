...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!accountSid || !authToken || !verifyServiceSid) {
      console.error("[report/kbml/send-sms] Twilio env missing. Set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_VERIFY_SERVICE_SID");
      return NextResponse.json(
        { ok: false, message: "SMS送信の設定がありません。管理者にお問い合わせください。" },
        { status: 500 }
      );
    }
    console.error("[report/kbml/send-sms] Twilio creds length", { sidLen: accountSid.length, tokenLen: authToken.length });
    ...
    try {
      ...
      console.error("[report/kbml/send-sms] Twilio error", { code, status, message: twilioErr instanceof Error ? twilioErr.message : String(twilioErr) });
    ...
    console.error("[report/kbml/send-sms] Unexpected error", err);
    return NextResponse.json(
