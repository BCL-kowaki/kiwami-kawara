...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【個別メール】【特別レポート申込】${name} 様`, adminBody);
    } else {
      console.log("[report/kbml/register] AWS not set, skipping email. Data:", { name, email, address });
    }

    return NextResponse.json({ ok: true, token });
  } catch (err) {
    console.error("[report/kbml/register]", err);
