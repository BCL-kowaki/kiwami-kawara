...
function formatAdminBody(data: ReportRegistrationBody): string {
  ...
  let body = `【特別レポート申込】受信データ（AIバブル崩壊）\n\n`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【特別レポート申込】${name} 様（AIバブル崩壊）`, adminBody);
    } else {
      console.log("[ai-bubble/register] AWS not set, skipping email. Data:", { name, email });
    }

    return NextResponse.json({ ok: true, token });
  } catch (err) {
    console.error("[ai-bubble/register]", err);
