...
function formatVerifiedAdminBody(name: string, email: string, address: string, phone: string): string {
  ...
  const addressLine = parts.slice(1).filter(Boolean).join(" ");
    let body = `【特別レポート申込】本人確認完了（AIバブル崩壊）\n\n`;
    ...
    export async function POST(request: NextRequest) {
      try {
        ...
        if (!accountSid || !authToken || !verifyServiceSid) {
          ...
          const phoneE164 = payload.phone.startsWith("+") ? payload.phone : `+81${payload.phone.replace(/^0/, "")}`;
        const client = Twilio(accountSid, authToken);
        const check = await client.verify.v2.services(verifyServiceSid).verificationChecks.create({ to: phoneE164, code: codeStr });
        if (check.status !== "approved") {
          return NextResponse.json({ ok: false, message: "認証コードが正しくないか、有効期限が切れています。" }, { status: 400 });
        }
        ...
        if (accessKeyId && secretAccessKey) {
          ...
          await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【特別レポート申込】本人確認完了 ${payload.name} 様（AIバブル崩壊）`, adminBody);
          await sendEmail(sesClient, fromEmail, payload.email, "【投賄のKAWARA版】AIバブル崩壊特別レポートのお申し込みを承りました", userBody);
        }
          return NextResponse.json({ ok: true });
      } catch (err) {
        console.error("[ai-bubble/verify-sms]", err);
