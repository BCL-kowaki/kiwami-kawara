...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【広告経由】【資産運用AI分析】本人確認完了 ${payload.name} 様`, adminBody);
      await sendEmail(sesClient, fromEmail, payload.email, "【投資のKAWARA版】資産分析AIエンジン“極”のお申し込みを承りました", userBody);
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[ad/verify-sms]", err);
