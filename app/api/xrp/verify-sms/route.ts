...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【特別レポート申込】本人確認完了 ${payload.name} 様（XRP）`, adminBody);
      await sendEmail(sesClient, fromEmail, payload.email, "【投資のKAWARA版】XRP特別レポートのお申し込みを承りました", userBody);
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[xrp/verify-sms]", err);
