...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!accountSid || !authToken || !verifyServiceSid) {
      console.error("[xrp/kbtm/send-sms] Twilio env missing.");
    ...
    try {
      ...
      console.error("[xrp/kbtm/send-sms] Twilio error", { code, status, message: twilioErr instanceof Error ? twilioErr.message : String(twilioErr) });
    ...
    console.error("[xrp/kbtm/send-sms] Unexpected error", err);
    return NextResponse.json(
