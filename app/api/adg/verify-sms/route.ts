...
function formatVerifiedAdminBody(name: string, email: string, phone: string): string {
  let body = `【Google広告経由】資産運用AI分析 本人確認完了\n\n`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      const adminBody = formatVerifiedAdminBody(payload.name, payload.email, payload.phone);
      const userBody = formatUserCompletedBody(payload.name);
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【Google広告経由】【資産運用AI分析】本人確認完了 ${payload.name} 様`, adminBody);
      await sendEmail(sesClient, fromEmail, payload.email, `【投資のKAWARA版】資産分析AIエンジン\u201C極\u201Dのお申し込みを承りました`, userBody);
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[adg/verify-sms]", err);
