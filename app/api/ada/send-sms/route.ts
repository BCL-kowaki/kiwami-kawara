...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!accountSid || !authToken || !verifyServiceSid) {
      console.error("[ada/send-sms] Twilio env missing.");
    ...
    try {
      ...
      console.error("[ada/send-sms] Twilio error", { code, status, message: twilioErr instanceof Error ? twilioErr.message : String(twilioErr) });
    ...
    console.error("[ada/send-sms] Unexpected error", err);
    return NextResponse.json(
