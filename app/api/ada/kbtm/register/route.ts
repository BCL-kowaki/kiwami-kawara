...
function formatAdminBody(data: ReportRegistrationBody): string {
  ...
  let body = `【個別メール】【特別レポート申込】受信データ（ADA）\n\n`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【個別メール】【特別レポート申込】${name} 様（ADA）`, adminBody);
    } else {
      console.log("[ada/kbtm/register] AWS not set, skipping email. Data:", { name, email });
    }

    return NextResponse.json({ ok: true, token });
  } catch (err) {
    console.error("[ada/kbtm/register]", err);
