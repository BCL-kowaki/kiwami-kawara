...
function formatVerifiedAdminBody(name: string, email: string, address: string, phone: string): string {
  ...
  let body = `【個別メール】【特別レポート申込】本人確認完了（ADA）\n\n`;
...
function formatUserCompletedBody(name: string): string {
  let body = `${name} 様\n\n`;
  body += `この度は、投資のKAWARA版ADA特別レポートに\n`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【個別メール】【特別レポート申込】本人確認完了 ${payload.name} 様（ADA）`, adminBody);
      await sendEmail(sesClient, fromEmail, payload.email, `【投資のKAWARA版】ADA特別レポートのお申し込みを承りました`, userBody);
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[ada/kbtm/verify-sms]", err);
