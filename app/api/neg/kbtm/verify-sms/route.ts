...
function formatVerifiedAdminBody(name: string, email: string, address: string, phone: string): string {
  ...
  let body = `【個別メール】【特別レポート申込】本人確認完了（BTC）\n\n`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【個別メール】【特別レポート申込】本人確認完了 ${payload.name} 様（BTC）`, adminBody);
      await sendEmail(sesClient, fromEmail, payload.email, `【投資のKAWARA版】BTC特別レポートのお申し込みを承りました`, userBody);
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[btc/kbtm/verify-sms]", err);
