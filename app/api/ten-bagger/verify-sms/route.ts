...
function formatVerifiedAdminBody(name: string, email: string, address: string, phone: string): string {
  ...
  let body = `【特別レポート申込】本人確認完了（テンバガー）\n\n`;
...
function formatUserCompletedBody(name: string): string {
  let body = `${name} 様\n\n`;
  body += `この度は、投資のKAWARA版テンバガー有望特別レポートに\n`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【特別レポート申込】本人確認完了 ${payload.name} 様（テンバガー）`, adminBody);
      await sendEmail(sesClient, fromEmail, payload.email, `【投資のKAWARA版】テンバガー有望特別レポートのお申し込みを承りました`, userBody);
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[ten-bagger/verify-sms]", err);
