...
export async function POST(request: NextRequest) {
  try {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      await sendEmail(sesClient, fromEmail, ADMIN_EMAILS, `【特別レポート申込】${name} 様（テンバガー）`, adminBody);
    } else {
      console.log("[ten-bagger/register] AWS not set, skipping email. Data:", { name, email });
    }

    return NextResponse.json({ ok: true, token });
  } catch (err) {
    console.error("[ten-bagger/register]", err);
