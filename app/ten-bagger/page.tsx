...
export default function TenBaggerPage() {
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/ten-bagger/register", {
        method: "POST",
  ...
  const handleSendSms = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/ten-bagger/send-sms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleVerify = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/ten-bagger/verify-sms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      try {
        ...
        if (typeof window !== "undefined" && (window as unknown as { gtag?: (...args: unknown[]) => void }).gtag) {
          (window as unknown as { gtag: (...args: unknown[]) => void }).gtag("event", "ten_bagger_report_verify_complete", {
            sender,
            ...
            <div className="p-[2px] rounded-[2px] shadow-2xl" style={borderStyle}>
              <div className="backdrop-blur-sm rounded-[2px] p-5 text-center" style={cardStyle}>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  投資のKAWARA版テンバガー有望特別レポートのお申し込みありがとうございます。
                  </p>
          ...
          <div className="backdrop-blur-sm rounded-[2px] p-4" style={cardStyle}>
            <div className="text-center mb-4">
              ...
              {step === "form" && (
                ...
                >
                  キオクシアで&ldquo;数十倍&rdquo;を逃した方へ<br />次に「10倍」を狙える銘柄は、すでに動き始めています。
                </h1>
              )}
              {/* コピー */}
...
キオクシアは、わずか1年で数十倍という異次元の成長を見せました。

今もなお、そんな圧倒的なポテンシャルを秘めた企業は、日本市場に存在するのか？

では同じような爆発は、もう起きないのでしょうか？

もしあの時、仕込めていたら…そう思ったことはありませんか？

同じようなチャンスは、もう二度と来ないのでしょうか？

答えは、NOです。

今回、投資の“KAWARA”版として”初の試み”を実施しました。

精密資産分析AIエンジン”極”をフル稼働させ、
「純粋に10倍以上を狙える超・爆発力を持った個別銘柄」を
一社限定で公開させていただきます。

実はこの銘柄は、

・すでに「国の承認」を取得
・製品価格は数千万円クラスと予想されている
・潜在市場は世界規模
・それにも関わらず、時価総額は約300億円

という、極めて歪なポジションにあります。

これらを正しく理解している投資家は、まだ少ないかもしれません。

つまり、今はまだ気づかれていない状況なのです。

このような銘柄は世間に知られた瞬間に一気に価格が跳ね上がりやすく、キオクシアも同じでした。

構造を見ずに表面的なニュースに踊らされているだけでは、チャンスをまた見逃してしまう可能性もあるかもしれません。

『時価総額300億円から10倍を狙う』AI“極”が導き出したテンバガー戦略レポート`
                }
                {step === "phone" &&
